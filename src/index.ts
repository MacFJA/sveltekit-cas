...
const getCasHelper = (root: string, version: 1 | 2 | 3, event: RequestEvent) => {
	return {
 ...
 *
 * export const handle: Handle = sequence(serverHook, casHandler(
 * 	 'http://0.0.0.0:8080',
 * 	 2,
...
export function casHandler(
	...
	return function (input: Parameters<Handle>[0]): ReturnType<Handle> {
		...
		switch (casVersion) {
			case 1:
				return cas1Hook(casRoot, input.event)
			case 2:
...
async function cas1Hook(casRoot: string, event: RequestEvent): Promise<Response> {
	const casHelper = getCasHelper(casRoot, 1, event)
...
async function cas23Hook(casRoot: string, version: 2 | 3, event: RequestEvent): Promise<Response> {
	...
	if ((ticket = getTicket(event)) !== undefined) {
		const validation: ServiceValidation = await event
			.fetch(casHelper.getValidationUrl(ticket))
			.then((response) => response.text())
			.then((response) => xml2js(response, { compact: true }) as ServiceValidation)
